...
X_train_normal = np.load("src/X_train.npy")
y_train = np.load("src/y_train.npy")

normal_mask = y_train == 0
X_train_normal = X_train_normal[normal_mask]
...
SUBSET_SIZE = 300000
X_seq_subset = X_seq[:SUBSET_SIZE]
print("Using subset:", X_seq_subset.shape)

X_tensor = torch.FloatTensor(X_seq_subset)
...
print("\nTraining complete.")
print(f"Best model: src/lstm_autoencoder_best.pth (val loss: {best_val_loss:.6f})") LSTMAutoencoder(input_size=11, hidden_size=32, bottleneck_size=8, num_layers=2)
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
loss_fn = nn.MSELoss()
...
for epoch in range(EPOCHS):
    ...
    # SAVE AFTER EVERY EPOCH — this is the fix
    torch.save(model.state_dict(), "src/lstm_autoencoder.pth")
    print(f"  Checkpoint saved (epoch {epoch+1})")

    # ALSO track and save the best model separately
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        torch.save(model.state_dict(), "src/lstm_autoencoder_best.pth")
        print(f"  New best model saved (val loss: {val_loss:.6f})")
...
print("\nTraining complete.")
print(f"Final model: src/lstm_autoencoder.pth")
